...
双击 `启动浏览器版.bat`：它会启动仅监听本机的 Python 服务与 React 前端，再以 Chromium 调试模式打开 `http://127.0.0.1:1420`。启动器窗口会保持运行；关闭它会结束本地服务。若启动失败，窗口会保留错误信息，详细的前端输出位于 `browser-launcher.log`。
## ⚠️ 使用声明
