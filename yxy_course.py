...
(function(config) {
  ...
  function forwardDialogTarget() {
    ...
    const dialogs = document.querySelectorAll('[role="dialog"], .modal, .dialog, .el-message-box, .ant-modal, .layui-layer, .popup');
    for (const dialog of dialogs) {
      if (!visible(dialog)) continue;
      const dialogText = compactText(dialog);
      if (/本人确认|身份验证|验证码|请确认在场/.test(dialogText)) continue;
      const controls = dialog.querySelectorAll('button, a, [role="button"]');
      for (const control of controls) {
        const text = compactText(control);
        if (!/^(继续学习|确定离开|继续\s*下一章(?:\s*>>)?|关闭|确定|继续)$/.test(text)) continue;
        const target = safeTarget(control, 'forward-dialog');
  ...
  function tick() {
    ...
    const quizUnfinished = quizUnfinishedCount();
    if (quizUnfinished > 0) {
      quizClearSince = 0;
      ...
      if (lastQuizNotice && quizUnfinished === 0) {
    ...
    const canFinishWithoutMedia = quizUnfinished === 0 && !videos.length &&
    ...
    if ((currentPageRecordComplete() || canFinishWithoutMedia) && quizSettled && Date.now() - stablePageSince >= 2500) {
      const staticKey = 'page-' + (state.page || stableHash(signature));
      if (staticPageNotice !== staticKey) {
        staticPageNotice = staticKey;
        event('static-ready', {source: staticKey, recordComplete: currentPageRecordComplete(), contentType: currentContentType()});
      }
    }

    if (courseFinished() && (currentPageRecordComplete() || quizUnfinished === 0 &&
      (allVideosFinished(videos) || canFinishWithoutMedia))) {
...
class PagePlan:
    ...
    def from_status(cls, status: dict[str, Any], machine: CourseStateMachine | None = None) -> "PagePlan":
        ...
        # 平台目录已给出已完成记录时，它是比播放器 UI 更可靠的完成判据。
        ...
        has_document = bool(status.get("hasDocument") or status.get("hasDocumentFrame"))
        document_done = bool(
            machine
        ...
        if has_document:
            tasks.append(PageTask("document", "completed" if document_done else "pending", 1))
        ...
        if quiz_count:
            tasks.append(PageTask("quiz", "pending", quiz_count, quiz_types or None))

        # 测验出现时先处理，避免视频和文档动作干扰可交互题目；其余按视频、文档顺序。
        active = next((task.kind for task in tasks if task.kind == "quiz" and task.state != "completed"), "")
        if not active:
            active = next((task.kind for task in tasks if task.state != "completed"), "navigation")
...
class CourseController:
    ...
    def status_snapshot(self) -> dict[str, Any]:
        ...
        task_names = {"video": "视频", "document": "文档", "quiz": "测验", "navigation": "翻页", "record": "翻页"}
    ...
    def _recover_stall(self, status: dict[str, Any]) -> None:
        if self.state_machine.state == CourseState.COMPLETED or self._course_completed_emitted:
            return
        state = status.get("state") if isinstance(status.get("state"), dict) else {}
        if int(status.get("quizUnfinished") or 0) > 0:
    ...
    def _attempt_navigation_if_ready(self, *, recovery: bool = False) -> None:
        ...
        if self._stall_active and not recovery:
            return
        status = self.status_snapshot()
        if status and status.get("courseFinished"):
            if self.state_machine.complete():
